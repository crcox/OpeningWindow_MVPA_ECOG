function [results,fitObj] = OpeningWindow_MVPA_ECOG(condition, subjects, holdouts, varargin)
...
% 2. sqrt_truncate_r.m (from WholeBrain_RSA/src)
    ...
    if isdeployed
        ...
        addParameter(p, 'windows', 50:10:2000);
        addParameter(p, 'parallel', 0);
        addParameter(p, 'dataroot', 'D:/ECoG/KyotoNaming/data/avg');
        addParameter(p, 'metadata', 'C:/Users/mbmhscc4/MATLAB/src/OpeningWindow_MVPA_ECOG/data/cvpartition_10fold_wholewindow.mat');
        parse(p, condition, subjects, holdouts, varargin{:});
        fprintf('condition: %s\n', p.Results.condition);
        fprintf('subjects: %d\n', p.Results.subjects);
        fprintf('holdouts: %d\n', p.Results.holdouts);
    ...
    for i = 1:N
        ...
        %  continue;
        %end
        datadir = fullfile(dataroot,'BoxCar','010','WindowStart','0000','WindowSize',sprintf('%04d', w));
